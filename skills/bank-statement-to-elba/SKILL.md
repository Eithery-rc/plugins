...
2. **Если `profile.use_elba=True`** — объясни и попроси ИНН для Эльба-матчинга:

   > Чтобы Эльба автоматически связывала поступления от этого контрагента с его карточкой в «Контрагенты», нужен ИНН, совпадающий в обоих местах.
   >
   > Для иностранных компаний (нет российского ИНН) правило такое:
   > 1. Открой Эльбу → раздел «Контрагенты» → создай или отредактируй карточку этого контрагента.
   > 2. В поле «ИНН» впиши **любое уникальное 10-12-значное число** (например, `9909999999` — главное, чтобы оно не совпадало с другими контрагентами у тебя).
   > 3. Сохрани карточку.
   > 4. Скажи мне ровно то же число — я запомню его, и все следующие импорты от этого контрагента будут цепляться к карточке автоматически (тип операции/описание/категория подтянутся из карточки).
   >
   > Если у компании **есть** реальный ИНН (российская фирма) — просто дай его. Это ещё надёжнее.
...
**При обработке выписки, если у контрагента в `contractors.json` поле `inn` пустое и `profile.use_elba=True`** — не продолжай молча. Предупреди пользователя: «У контрагента X в профиле пустой ИНН. Эльба не сможет связать поступления с карточкой. Укажи placeholder-ИНН сейчас или подтверди, что ок генерить с `0000000000` (тогда при импорте в Эльбу контрагенты не свяжутся — каждый раз будут создаваться новые).»
